# Snow Beer Packaging V1 Precedence High-Utilization Sample Package

This package stays close to `snow_beer_packaging_v1_precedence` but changes three assumptions so the current baseline can demonstrate a tighter, higher-utilization schedule:

- `objectiveWeights.earliness` is set to `0`
- delayed packaging-material availability checkpoints are moved to `0`
- synthetic downtime windows are removed

It keeps the same packaging-only scope, the same demand slices, the same two-step `PREP -> PACK` recipe chain, and the same setup rules as the precedence baseline.
...
- this high-utilization variant removes delayed material-availability checkpoints for the selected packaging materials
- this high-utilization variant removes synthetic downtime rows so the baseline can focus on tighter schedule compaction
...
- `05_downtimes.csv`: empty in this variant so utilization is not diluted by synthetic service windows
- `06_setup_rules.csv`: `330/500/600ml` cross-volume changeovers
- `07_data_issue_log.csv`: current modeling compromises and gaps
- `14_inventory_balances.csv`: finished goods, bright beer proxies, and packaging material proxy stock with selected delayed arrivals moved to time `0`
...
```bash
rtk python3 scripts/build_uat_sample_package.py docs/sample-packages/snow_beer_packaging_v1_precedence_high_util --check
```

Generate derived artifacts:

```bash
rtk python3 scripts/build_uat_sample_package.py docs/sample-packages/snow_beer_packaging_v1_precedence_high_util
```

Optional backend smoke:

```bash
rtk python3 scripts/uat_package_smoke.py docs/sample-packages/snow_beer_packaging_v1_precedence_high_util
```
...
- compare a higher-utilization objective setup against the precedence baseline on the same demand and recipe structure
